...
import { formatAddress, formatAmount } from '../../lib/kleros';
...
const TransactionTimeline = ({ transactionEvents, transaction }: TransactionTimelineProps) => {
  if (!transactionEvents) return null;
  ...
  if (transactionEvents.rulings && transactionEvents.rulings.length > 0) {
    transactionEvents.rulings.forEach((ruling: any, index: number) => {
      timelineEvents.push({
        type: 'ruling',
        title: "Ruling Given",
